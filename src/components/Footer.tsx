...
export function Footer() {
  ...
  return (
    <footer className="bg-madia-black text-madia-white border-t-2 border-madia-gold/70">

      {/* ── MOBILE LAYOUT ── */}
      <div className="md:hidden px-6 pt-10 pb-8 flex flex-col items-center gap-0">

        {/* Logo centrato */}
        <img src="/mtlogo-removebg-preview.png" alt="Madia Teramo" className="h-16 w-auto object-contain mb-5" />

        {/* Nav links su una riga */}
        <div className="flex flex-wrap justify-center gap-x-5 gap-y-2 mb-6">
          {[
            ...
            <Link key={label} to={to} onClick={(e) => handleNavClick(e, to)} className="text-[10px] uppercase tracking-[0.2em] text-madia-white/60 hover:text-madia-gold transition-colors duration-300">
              {label}
            </Link>
          ))}
        </div>
        ...
        {/* Divider */}
        <div className="w-full border-t border-madia-gold/20 mb-5" />

        {/* Testo legale */}
        <p className="uppercase tracking-[0.12em] text-[9px] text-center text-madia-white/30 leading-relaxed mb-5">
          © {new Date().getFullYear()} Madia Teramo S.r.l. · Tutti i diritti riservati<br />
          P.IVA 02199950672 · REA TE-216820<br />
          Cap. Soc. € 10.000,00 i.v.<br />
          PEC madiateramosrl@pec.it
        </p>
        ...
        {/* Documenti legali */}
        <div className="flex flex-wrap justify-center items-center gap-x-3 gap-y-2 uppercase tracking-[0.15em] text-[9px] text-madia-white/30">
      ...
      {/* ── DESKTOP LAYOUT ── */}
      <div className="hidden md:block px-12 pt-8 pb-8">
          ...
          <div className="flex justify-between items-center pb-6">
            ...
            <div className="flex flex-wrap justify-center gap-6">
              {[
                ...
                <Link key={label} to={to} onClick={(e) => handleNavClick(e, to)} className="text-[9px] uppercase tracking-[0.2em] text-madia-white/50 hover:text-madia-gold transition-colors duration-300">
                  {label}
                </Link>
              ))}
            </div>
